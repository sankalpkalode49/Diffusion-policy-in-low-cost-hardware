...
class DiffusionDeploymentNode(Node):
    def __init__(self):
        ...
        # Normalization bounds for denormalization (Must match Dataloader exactly)
        ...
        # ==========================================
        # 2. NEURAL NETWORK & WEIGHT LOADING
        # ==========================================
        self.get_logger().info("🧠 Loading Neural Network weights...")
        self.vision_encoder = VisionEncoder().to(self.device)
        self.unet = ConditionalUNet1D(action_dim=4).to(self.device)
        self.noise_scheduler = DDPMScheduler(num_timesteps=100)

        # 🔵 CHECKPOINT TESTER: Change this to "30", "40", "50", or "final"
        EPOCH_TO_TEST = "50"  
        ...
        self.vision_encoder.load_state_dict(torch.load(os.path.join(save_dir, vis_weight)))
        self.unet.load_state_dict(torch.load(os.path.join(save_dir, unet_weight)))
    ...
    def get_live_image_tensor(self):
        ...
        # Display the live feed so you can ensure it matches training
        cv2.imshow("Diffusion Policy Autonomous View", frame)
        cv2.waitKey(1)

        # Pass the native 640x480 image to match training
    ...
    def run_autonomous_loop(self):
        with torch.no_grad(): # No gradients needed for inference!
            while rclpy.ok():
                ...
                obs_features = self.vision_encoder(img_tensor)
                ...
                for t in reversed(range(self.noise_scheduler.num_timesteps)):
                    ...
                    # 🔴 INFERENCE=TRUE: Stops stochastic chattering!
                ...
                # 🔴 THE FIX: Skip steps 0-3 (the hesitation). Execute steps 4-7 (the commitment).
